...
st.set_page_config(page_title="Growth Blueprint V28", layout="wide")
...
if not df.empty and all(a in df.columns for a in final_assets) and bench_ticker in df.columns:
    ...
    if not df_clean.empty:
        ...
        # PERBAIKAN LOGIKA STATUS (3 TAHAP)
        # 1. Sharpe Ratio
        sharpe = ((p_ret.mean() - (0.04/252)) / p_ret.std()) * np.sqrt(252)
        if sharpe > 1.5: s_n = "🔵 Baik"
        elif sharpe >= 1.0: s_n = "🟡 Sedang"
        else: s_n = "🔴 Kurang"
        
        # 2. Alpha
        alpha = (p_ret.mean() * 252) - (0.04 + (p_ret.cov(b_ret)/b_ret.var()) * ((b_ret.mean() * 252) - 0.04))
        alpha_pct = alpha * 100
        if alpha_pct > 5.0: a_n = "🔵 Baik"
        elif alpha_pct >= 0.0: a_n = "🟡 Sedang"
        else: a_n = "🔴 Kurang"
        
        # 3. Drawdown
        dd = (p_cum / p_cum.cummax()) - 1
        max_dd = dd.min()
        if max_dd >= -0.15: d_n = "🔵 Baik"
        elif max_dd >= -0.30: d_n = "🟡 Sedang"
        else: d_n = "🔴 Kurang"
        ...
        # Pemanggilan menggunakan variabel status yang sudah diperbaiki
        with m2: st.markdown(kpi_box("Sharpe Ratio (Efficiency)", f"{sharpe:.2f}", s_n, "🔴 <1.0 | 🟡 1.0-1.5 | 🔵 >1.5"), unsafe_allow_html=True)
        with m3: st.markdown(kpi_box("Alpha vs Benchmark", f"{alpha_pct:.1f}%", a_n, "🔴 <0% | 🟡 0-5% | 🔵 >5%"), unsafe_allow_html=True)
        with m4: st.markdown(kpi_box("Max Drawdown", f"{max_dd*100:.2f}%", d_n, "🔵 >-15% | 🟡 -30% | 🔴 <-30%"), unsafe_allow_html=True)
