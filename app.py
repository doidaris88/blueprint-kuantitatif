...
st.set_page_config(page_title="Growth Blueprint V2", layout="wide")
...
# Fokus pada konsentrasi di aset utama untuk akselerasi maksimal
def get_ref_weight(index, total):
    if total == 5:
        return [35.0, 25.0, 20.0, 10.0, 10.0][index]
    else:
        # Distribusi menurun otomatis jika jumlah aset berbeda
        decay = 0.7 ** index
...
for i in range(num_assets):
    ...
    with col_a:
        ...
        # Auto-Capitalization menggunakan .upper()
...
@st.cache_data(ttl=3600) # Cache 1 jam untuk efisiensi
...
if not data.empty and all(a in data.columns for a in assets):
    ...
    m1.metric("Nilai Portofolio", f"${current_value:.2f}")
    ...
    fig.update_layout(height=450, template="plotly_dark", margin=dict(l=10, r=10, t=10, b=10),
                      hovermode=False, xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor='rgba(255,255,255,0.05)'),
