...
for i in range(min(3, st.session_state.num_assets)):
    ...
    with col_a:
        t = st.text_input(f"t{i}", value=def_val, key=f"t_{i}", label_visibility="collapsed").strip().upper()
    with col_w:
...
if not df.empty and all(a in df.columns for a in assets) and bench_ticker in df.columns:
    ...
    if not df_clean.empty:
        ...
        st.error("Data saham tidak sejajar (ada saham baru IPO). Kurangi jumlah aset atau ganti ticker.")
else:
