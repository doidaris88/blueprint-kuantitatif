...
st.set_page_config(page_title="Growth Blueprint V13", layout="wide")
...
st.markdown("""
    ...
    /* Memaksa input teks selalu huruf kapital */
    input[type=text] {
        text-transform: uppercase;
    }

    /* Pengaturan Font Judul Besar & Simetris */
    .main-title {
...
for i in range(st.session_state.num_assets):
    ...
    container = st.sidebar if i < 3 else st.sidebar.expander("⬇️ Aset Tambahan")
...
def fetch_data(tickers, benchmark):
    ...
    try:
        return yf.download(all_t, period="3y", progress=False)['Close']
    except:
        return pd.DataFrame()

df = fetch_data(assets, bench_ticker)

if not df.empty and all(a in df.columns for a in assets):
    ...
    def kpi_box(label, val, status, legend):
        ...
        <div style="background-color: #ffffff; padding: 18px; border-radius: 10px; border: 1px solid #eeeeee; height: 100%;">
            ...
            <div style="display: flex; align-items: baseline;">
                <div style="font-size: 34px; font-weight: bold; color: #111111;">{val}</div>
                <div style="font-size: 14px; margin-left: 12px; font-weight: 600; color: #666666;">{status}</div>
            </div>
    ...
    with g1:
        ...
        f1.add_trace(go.Scatter(x=b_cum.index, y=b_cum, name='Benchmark', line=dict(color='#bbbbbb', width=1.5)))
